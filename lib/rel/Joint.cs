...
namespace nilnul.collection.set.rel
{
	...
	{
		static public nilnul.bit.Predicate<HashSet<T>, HashSet<T>> CreateAssert<T>()
		{
			return new Predicate<HashSet<T>, HashSet<T>>(Is);
		}

		static public nilnul.bit.Predicate<HashSet<T>, HashSet<T>> CreateDeny<T>()
		{
		...
		static public class Singleton<T>
		{
			...
			static public nilnul.bit.Predicate<HashSet<HashSet<T>>> enumerableDeny = nilnul.bit.Predicate<HashSet<HashSet<T>>>.CreateInverse(Is);

		}

		static public nilnul.bit.Predicate<HashSet<HashSet<T>>> CreateEnumerableDeny<T>()
		{
		...
		static public bool Is<T, Eq>(Set<T, Eq> a, Set<T, Eq> b)
		...
		static public bool Is<T, Eq>(Union2<T, Eq> a, Set<T, Eq> b)
			where Eq : IEqualityComparer<T>, new()
		{
			return Is(a.ToLiteral(), b);
		...
		static public bool Is<T, Eq>(Set<T, Eq> b, Union2<T, Eq> a)
	where Eq : IEqualityComparer<T>, new()
		{
		...
		static public bool Is<TElement, Eq>(TElement element, Set<TElement, Eq> sets)
			where Eq : IEqualityComparer<TElement>, new()
		{
			return sets.Contains(element);
		}

		static public bool Is<TElement, Eq>(IEnumerable<Set<TElement, Eq>> sets)
		...
		static public bool _Is<TElement, Eq>(IEnumerable<Set<TElement, Eq>> sets_multiple)
			where Eq : IEqualityComparer<TElement>, new()
		{
		...
		static public bool No<TElement, TEq>(IEnumerable<Set<TElement, TEq>> sets)
		...
		static public void AssertIs<T, Eq>(Union2<T, Eq> a, Set<T, Eq> b)
			where Eq : IEqualityComparer<T>, new()
		{
			nilnul.bit.AssertX.True(Is<T, Eq>(a, b));

		}



		static public void AssertNo<T, Eq>(Union2<T, Eq> a, Set<T, Eq> b)
			where Eq : IEqualityComparer<T>, new()
		{
